...
import "../../interface/dao/protocol/settings/SafeStakeDAOProtocolSettingsInflationInterface.sol";
...
contract SafeStakeTokenRPL is SafeStakeBase, ERC20Burnable, SafeStakeTokenRPLInterface {
    ...
    // Construct
    constructor(SafeStakeStorageInterface _safeStakeStorageAddress, IERC20 _safeStakeTokenRPLFixedSupplyAddress) SafeStakeBase(_safeStakeStorageAddress) ERC20("SafeStake Pool Protocol", "RPL") {
        // Version
        version = 1;
        // Set the mainnet RPL fixed supply token address
        rplFixedSupplyContract = IERC20(_safeStakeTokenRPLFixedSupplyAddress);
    ...
    */
    function getInflationIntervalRate() override public view returns(uint256) {
        ...
        SafeStakeDAOProtocolSettingsInflationInterface daoSettingsInflation = SafeStakeDAOProtocolSettingsInflationInterface(getContractAddress("safeStakeDAOProtocolSettingsInflation"));
    ...
    */
    function getInflationRewardsContractAddress() override external view returns(address) {
        ...
        return getContractAddress("safeStakeRewardsPool");
    }
    ...
    */
    function inflationMintTokens() override external returns (uint256) {
        ...
        require(safeStakeVaultAddress != address(0x0), "safeStakeVault address not set");
        // Only mint if we have new tokens to mint since last interval and an address is set to receive them
        SafeStakeVaultInterface safeStakeVaultContract = SafeStakeVaultInterface(safeStakeVaultAddress);
        ...
        if (newTokens > 0) {
            ...
            // Get the current allowance for SafeStake Vault
            uint256 vaultAllowance = rplFixedSupplyContract.allowance(safeStakeVaultAddress, address(this));
            // Now allow SafeStake Vault to move those tokens, we also need to account of any other allowances for this token from other contracts in the same block
            require(rplInflationContract.approve(safeStakeVaultAddress, vaultAllowance.add(newTokens)), "Allowance for SafeStake Vault could not be approved");
            // Let vault know it can move these tokens to itself now and credit the balance to the RPL rewards pool contract
            safeStakeVaultContract.depositToken("safeStakeRewardsPool", IERC20(address(this)), newTokens);
        }
        // Log it
