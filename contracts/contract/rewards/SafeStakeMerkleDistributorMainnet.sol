...
contract SafeStakeMerkleDistributorMainnet is SafeStakeBase, SafeStakeRewardsRelayInterface, SafeStakeVaultWithdrawerInterface {
    ...
    // Immutables
    bytes32 immutable safeStakeVaultKey;
    bytes32 immutable safeStakeTokenRPLKey;
    ...
    constructor(SafeStakeStorageInterface _safeStakeStorageAddress) SafeStakeBase(_safeStakeStorageAddress) {
        ...
        version = 1;
        // Precompute keys
        safeStakeVaultKey = keccak256(abi.encodePacked("contract.address", "safeStakeVault"));
        safeStakeTokenRPLKey = keccak256(abi.encodePacked("contract.address", "safeStakeTokenRPL"));
    ...
    // Called by SafeStakeRewardsPool to include a snapshot into this distributor
    function relayRewards(uint256 _rewardIndex, bytes32 _root, uint256 _rewardsRPL, uint256 _rewardsETH) external override onlyLatestContract("safeStakeMerkleDistributorMainnet", address(this)) onlyLatestContract("safeStakeRewardsPool", msg.sender) {
        ...
        SafeStakeVaultInterface safeStakeVault = SafeStakeVaultInterface(getAddress(safeStakeVaultKey));
        if (_rewardsETH > 0) {
            safeStakeVault.depositEther{value: _rewardsETH}();
        }
        if (_rewardsRPL > 0) {
            IERC20 safeStakeTokenRPL = IERC20(getAddress(safeStakeTokenRPLKey));
            safeStakeTokenRPL.approve(address(safeStakeVault), _rewardsRPL);
            safeStakeVault.depositToken("safeStakeMerkleDistributorMainnet", safeStakeTokenRPL, _rewardsRPL);
        }
    }
    ...
    function claimAndStake(address _nodeAddress, uint256[] calldata _rewardIndex, uint256[] calldata _amountRPL, uint256[] calldata _amountETH, bytes32[][] calldata _merkleProof, uint256 _stakeAmount) public override {
        // Get contracts
        SafeStakeVaultInterface safeStakeVault = SafeStakeVaultInterface(getAddress(safeStakeVaultKey));
        address safeStakeTokenRPLAddress = getAddress(safeStakeTokenRPLKey);
        // Verify claims
        _claim(_rewardIndex, _nodeAddress, _amountRPL, _amountETH, _merkleProof);
        {
            ...
            if (remaining > 0) {
                safeStakeVault.withdrawToken(withdrawalAddress, IERC20(safeStakeTokenRPLAddress), remaining);
            }
            // Distribute ETH
            if (totalAmountETH > 0) {
                safeStakeVault.withdrawEther(totalAmountETH);
        ...
        if (_stakeAmount > 0) {
            SafeStakeTokenRPLInterface safeStakeTokenRPL = SafeStakeTokenRPLInterface(safeStakeTokenRPLAddress);
            SafeStakeNodeStakingInterface safeStakeNodeStaking = SafeStakeNodeStakingInterface(getContractAddress("safeStakeNodeStaking"));
            safeStakeVault.withdrawToken(address(this), IERC20(safeStakeTokenRPLAddress), _stakeAmount);
            safeStakeTokenRPL.approve(address(safeStakeNodeStaking), _stakeAmount);
            safeStakeNodeStaking.stakeRPLFor(_nodeAddress, _stakeAmount);
        }
        // Emit event
    ...
    // Allow receiving ETH from SafeStakeVault, no action required
