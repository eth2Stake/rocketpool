...
contract SafeStakeMinipoolPenalty is SafeStakeBase, SafeStakeMinipoolPenaltyInterface {
    ...
    // Storage (purposefully does not use SafeStakeStorage to prevent oDAO from having power over this feature)
    ...
    function getPenaltyRate(address _minipoolAddress) external override view returns(uint256) {
        // Quick out which avoids a call to SafeStakeStorage
        if (maxPenaltyRate == 0) {
             return 0;
        }
