...
contract SafeStakeMinipoolDelegate is SafeStakeMinipoolStorageLayout, SafeStakeMinipoolInterface {
    ...
    modifier onlyMinipoolOwnerOrWithdrawalAddress(address _nodeAddress) {
        require(_nodeAddress == nodeAddress || _nodeAddress == safeStakeStorage.getNodeWithdrawalAddress(nodeAddress), "Invalid minipool owner");
        _;
    }

    // Only allow access from the latest version of the specified SafeStake Pool contract
    ...
    function nodeDeposit(bytes calldata _validatorPubkey, bytes calldata _validatorSignature, bytes32 _depositDataRoot) override external payable onlyLatestContract("safeStakeNodeDeposit", msg.sender) onlyInitialised {
    ...
    function userDeposit() override external payable onlyLatestContract("safeStakeDepositPool", msg.sender) onlyInitialised {
    ...
    function canStake() override external view onlyInitialised returns (bool) {
        ...
        // Get contracts
        SafeStakeDAONodeTrustedSettingsMinipoolInterface safeStakeDAONodeTrustedSettingsMinipool = SafeStakeDAONodeTrustedSettingsMinipoolInterface(getContractAddress("safeStakeDAONodeTrustedSettingsMinipool"));
        // Get scrub period
        uint256 scrubPeriod = safeStakeDAONodeTrustedSettingsMinipool.getScrubPeriod();
    ...
    function stake(bytes calldata _validatorSignature, bytes32 _depositDataRoot) override external onlyMinipoolOwner(msg.sender) onlyInitialised {
        ...
        // Get launch amount
        uint256 launchAmount = safeStakeDAOProtocolSettingsMinipool.getLaunchBalance().sub(prelaunchAmount);
        ...
        bytes memory validatorPubkey = safeStakeMinipoolManager.getMinipoolPubkey(address(this));
        // Send staking deposit to casper
        casperDeposit.deposit{value : launchAmount}(validatorPubkey, safeStakeMinipoolManager.getMinipoolWithdrawalCredentials(address(this)), _validatorSignature, _depositDataRoot);
        // Increment node's number of staking minipools
        safeStakeMinipoolManager.incrementNodeStakingMinipoolCount(nodeAddress);
    }
    ...
    function preStake(bytes calldata _validatorPubkey, bytes calldata _validatorSignature, bytes32 _depositDataRoot) internal {
        ...
        require(safeStakeMinipoolManager.getMinipoolByPubkey(_validatorPubkey) == address(0x0), "Validator pubkey is in use");
        // Set minipool pubkey
        safeStakeMinipoolManager.setMinipoolPubkey(_validatorPubkey);
        // Get withdrawal credentials
        bytes memory withdrawalCredentials = safeStakeMinipoolManager.getMinipoolWithdrawalCredentials(address(this));
        // Send staking deposit to casper
    ...
    // Only accepts calls from the SafeStakeMinipoolStatus contract
    function setWithdrawable() override external onlyLatestContract("safeStakeMinipoolStatus", msg.sender) onlyInitialised {
        // Get contracts
        SafeStakeMinipoolManagerInterface safeStakeMinipoolManager = SafeStakeMinipoolManagerInterface(getContractAddress("safeStakeMinipoolManager"));
        // Check current status
        ...
        if (userDepositAssignedTime == 0) {
            ...
            safeStakeMinipoolQueue.removeMinipool(depositType);
        }
        // Decrement the node operator's staking minipool count
        safeStakeMinipoolManager.decrementNodeStakingMinipoolCount(nodeAddress);
    }
    ...
    function distributeBalance() override external onlyInitialised {
        ...
        address nodeWithdrawalAddress = safeStakeStorage.getNodeWithdrawalAddress(nodeAddress);
    ...
    function _finalise() private {
        ...
        if (depositType == MinipoolDeposit.Empty) {
            ...
            safeStakeDAONodeTrusted.decrementMemberUnbondedValidatorCount(nodeAddress);
        }
        // Set finalised flag
        finalised = true;
    }
    ...
    function calculateNodeShare(uint256 _balance) override public view returns (uint256) {
        ...
        uint256 penaltyRate = SafeStakeMinipoolPenaltyInterface(safeStakeMinipoolPenalty).getPenaltyRate(address(this));
        if (penaltyRate > 0) {
    ...
    function dissolve() override external onlyInitialised {
        ...
        require(
            (status == MinipoolStatus.Prelaunch && block.timestamp.sub(statusTime) >= safeStakeDAOProtocolSettingsMinipool.getLaunchTimeout()),
    ...
    function voteScrub() override external onlyInitialised {
        ...
        require(safeStakeDAONode.getMemberIsValid(msg.sender), "Not a trusted member");
        // Can only vote once
        ...
        uint256 quorum = safeStakeDAONode.getMemberCount().mul(safeStakeDAONodeTrustedSettingsMinipool.getScrubQuorum()).div(calcBase);
    ...
    function _dissolve() private {
        ...
        if (userDepositBalance > 0) {
            ...
            // Transfer
            safeStakeDepositPool.recycleDissolvedDeposit{value : recycleAmount}();
            // Emit ether withdrawn event
            emit EtherWithdrawn(address(safeStakeDepositPool), recycleAmount, block.timestamp);
        } else {
            safeStakeMinipoolQueue.removeMinipool(depositType);
        }
    }
