...
contract SafeStakeMinipoolQueue is SafeStakeBase, SafeStakeMinipoolQueueInterface {
    ...
    function getTotalCapacity() override external view returns (uint256) {
        ...
        return (
            getLength(queueKeyFull).mul(safeStakeDAOProtocolSettingsMinipool.getFullDepositUserAmount())
        ).add(
            getLength(queueKeyHalf).mul(safeStakeDAOProtocolSettingsMinipool.getHalfDepositUserAmount())
        ).add(
            getLength(queueKeyQuarter).mul(safeStakeDAOProtocolSettingsMinipool.getQuarterDepositUserAmount())
        ).add(
            getLength(queueKeyEmpty).mul(safeStakeDAOProtocolSettingsMinipool.getEmptyDepositUserAmount())
        );
    }
    ...
    function getNextCapacity() override external view returns (uint256) {
        SafeStakeDAOProtocolSettingsMinipoolInterface safeStakeDAOProtocolSettingsMinipool = SafeStakeDAOProtocolSettingsMinipoolInterface(getContractAddress("safeStakeDAOProtocolSettingsMinipool"));
        if (getLength(queueKeyHalf) > 0) { return safeStakeDAOProtocolSettingsMinipool.getHalfDepositUserAmount(); }
        if (getLength(queueKeyFull) > 0) { return safeStakeDAOProtocolSettingsMinipool.getFullDepositUserAmount(); }
        if (getLength(queueKeyQuarter) > 0) { return safeStakeDAOProtocolSettingsMinipool.getQuarterDepositUserAmount(); }
        if (getLength(queueKeyEmpty) > 0) { return safeStakeDAOProtocolSettingsMinipool.getEmptyDepositUserAmount(); }
        return 0;
    }
    ...
    function enqueueMinipool(MinipoolDeposit _depositType, address _minipool) override external onlyLatestContract("safeStakeMinipoolQueue", address(this)) onlyLatestContract("safeStakeMinipoolManager", msg.sender) {
    ...
    function dequeueMinipool() override external onlyLatestContract("safeStakeMinipoolQueue", address(this)) onlyLatestContract("safeStakeDepositPool", msg.sender) returns (address minipoolAddress) {
    ...
    function dequeueMinipoolByDeposit(MinipoolDeposit _depositType) override external onlyLatestContract("safeStakeMinipoolQueue", address(this)) onlyLatestContract("safeStakeDepositPool", msg.sender) returns (address minipoolAddress) {
    ...
    function removeMinipool(MinipoolDeposit _depositType) override external onlyLatestContract("safeStakeMinipoolQueue", address(this)) onlyRegisteredMinipool(msg.sender) {
