...
import "../../interface/auction/SafeStakeAuctionManagerInterface.sol";
import "../../interface/deposit/SafeStakeDepositPoolInterface.sol";
import "../../interface/network/SafeStakeNetworkPricesInterface.sol";
import "../../interface/dao/protocol/settings/SafeStakeDAOProtocolSettingsAuctionInterface.sol";
import "../../interface/SafeStakeVaultInterface.sol";

// Facilitates RPL liquidation auctions

contract SafeStakeAuctionManager is SafeStakeBase, SafeStakeAuctionManagerInterface {
    ...
    function getTotalRPLBalance() override public view returns (uint256) {
        ...
        return safeStakeVault.balanceOfToken("safeStakeAuctionManager", IERC20(getContractAddress("safeStakeTokenRPL")));
    }
    ...
    function createLot() override external onlyLatestContract("safeStakeAuctionManager", address(this)) {
        // Load contracts
        SafeStakeDAOProtocolSettingsAuctionInterface safeStakeAuctionSettings = SafeStakeDAOProtocolSettingsAuctionInterface(getContractAddress("safeStakeDAOProtocolSettingsAuction"));
        ...
        uint256 rplPrice = safeStakeNetworkPrices.getRPLPrice();
        // Check lot can be created
        require(safeStakeAuctionSettings.getCreateLotEnabled(), "Creating lots is currently disabled");
        require(remainingRplBalance >= calcBase.mul(safeStakeAuctionSettings.getLotMinimumEthValue()).div(rplPrice), "Insufficient RPL balance to create new lot");
        // Calculate lot RPL amount
        uint256 lotRplAmount = remainingRplBalance;
        uint256 maximumLotRplAmount = calcBase.mul(safeStakeAuctionSettings.getLotMaximumEthValue()).div(rplPrice);
        ...
        setUint(keccak256(abi.encodePacked("auction.lot.block.end", lotIndex)), block.number.add(safeStakeAuctionSettings.getLotDuration()));
        setUint(keccak256(abi.encodePacked("auction.lot.price.start", lotIndex)), rplPrice.mul(safeStakeAuctionSettings.getStartingPriceRatio()).div(calcBase));
        setUint(keccak256(abi.encodePacked("auction.lot.price.reserve", lotIndex)), rplPrice.mul(safeStakeAuctionSettings.getReservePriceRatio()).div(calcBase));
    ...
    // Bid on a lot
    function placeBid(uint256 _lotIndex) override external payable onlyLatestContract("safeStakeAuctionManager", address(this)) {
        // Load contracts
        SafeStakeDAOProtocolSettingsAuctionInterface safeStakeAuctionSettings = SafeStakeDAOProtocolSettingsAuctionInterface(getContractAddress("safeStakeDAOProtocolSettingsAuction"));
        ...
        require(safeStakeAuctionSettings.getBidOnLotEnabled(), "Bidding on lots is currently disabled");
        ...
        safeStakeDepositPool.recycleLiquidatedStake{value: bidAmount}();
    ...
    function claimBid(uint256 _lotIndex) override external onlyLatestContract("safeStakeAuctionManager", address(this)) {
        // Check lot exists
        require(getLotExists(_lotIndex), "Lot does not exist");
        // Get lot price info
        ...
        SafeStakeVaultInterface safeStakeVault = SafeStakeVaultInterface(getContractAddress("safeStakeVault"));
        safeStakeVault.withdrawToken(msg.sender, IERC20(getContractAddress("safeStakeTokenRPL")), rplAmount);
    ...
    function recoverUnclaimedRPL(uint256 _lotIndex) override external onlyLatestContract("safeStakeAuctionManager", address(this)) {
