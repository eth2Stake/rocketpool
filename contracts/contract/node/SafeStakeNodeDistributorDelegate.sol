...
contract SafeStakeNodeDistributorDelegate is SafeStakeNodeDistributorStorageLayout, SafeStakeNodeDistributorInterface {
    ...
    bytes32 immutable safeStakeTokenSFETHKey;

    constructor() {
        // Precompute storage keys
        safeStakeNodeManagerKey = keccak256(abi.encodePacked("contract.address", "safeStakeNodeManager"));
        safeStakeTokenSFETHKey = keccak256(abi.encodePacked("contract.address", "safeStakeTokenSFETH"));
    ...
    function distribute() override external {
        ...
        address safeStakeTokenSFETH = safeStakeStorage.getAddress(safeStakeTokenSFETHKey);
        ...
        // Transfer user share
        payable(safeStakeTokenSFETH).transfer(userShare);
        // Transfer node share
