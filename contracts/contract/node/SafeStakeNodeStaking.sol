...
contract SafeStakeNodeStaking is SafeStakeBase, SafeStakeNodeStakingInterface {
    ...
    function getTotalEffectiveRPLStake() override external view returns (uint256) {
        ...
        return safeStakeNetworkPrices.getEffectiveRPLStake();
    }
    ...
    function calculateTotalEffectiveRPLStake(uint256 offset, uint256 limit, uint256 rplPrice) override external view returns (uint256) {
        ...
        for (uint i = offset; i < max; i++){
            ...
            uint256 maxRplStake = maxRplStakePerMinipool.mul(safeStakeMinipoolManager.getNodeStakingMinipoolCount(nodeAddress)).div(rplPrice);
    ...
    function getNodeEffectiveRPLStake(address _nodeAddress) override external view returns (uint256) {
        ...
        // Calculate node's maximum RPL stake
        uint256 maxRplStake = safeStakeDAOProtocolSettingsMinipool.getHalfDepositUserAmount()
            .mul(safeStakeDAOProtocolSettingsNode.getMaximumPerMinipoolStake())
            .mul(safeStakeMinipoolManager.getNodeStakingMinipoolCount(_nodeAddress))
    ...
    function getNodeMinimumRPLStake(address _nodeAddress) override external view returns (uint256) {
        ...
        return safeStakeDAOProtocolSettingsMinipool.getHalfDepositUserAmount()
            .mul(safeStakeDAOProtocolSettingsNode.getMinimumPerMinipoolStake())
            .mul(safeStakeMinipoolManager.getNodeActiveMinipoolCount(_nodeAddress))
            .div(safeStakeNetworkPrices.getRPLPrice());
    }
    ...
    function getNodeMinipoolLimit(address _nodeAddress) override external view returns (uint256) {
        ...
        return getNodeRPLStake(_nodeAddress)
            .mul(safeStakeNetworkPrices.getRPLPrice())
            .div(
    ...
    function stakeRPL(uint256 _amount) override external onlyLatestContract("safeStakeNodeStaking", address(this)) onlyRegisteredNode(msg.sender) {
        _stakeRPL(msg.sender, _amount);
    }

    // Accept an RPL stake from any address for a specified node
    function stakeRPLFor(address _nodeAddress, uint256 _amount) override external onlyLatestContract("safeStakeNodeStaking", address(this)) onlyRegisteredNode(_nodeAddress) {
        _stakeRPL(_nodeAddress, _amount);
    }

    function _stakeRPL(address _nodeAddress, uint256 _amount) internal {
        // Load contracts
        address rplTokenAddress = getContractAddress("safeStakeTokenRPL");
        ...
        require(rplToken.approve(safeStakeVaultAddress, _amount), "Could not approve vault RPL deposit");
        safeStakeVault.depositToken("safeStakeNodeStaking", rplToken, _amount);
    ...
    // Only accepts calls from registered nodes
    function withdrawRPL(uint256 _amount) override external onlyLatestContract("safeStakeNodeStaking", address(this)) onlyRegisteredNode(msg.sender) {
        // Load contracts
        ...
        require(block.timestamp.sub(getNodeRPLStakedTime(msg.sender)) >= safeStakeDAOProtocolSettingsRewards.getRewardsClaimIntervalTime(), "The withdrawal cooldown period has not passed");
        ...
        safeStakeVault.withdrawToken(safeStakeStorage.getNodeWithdrawalAddress(msg.sender), IERC20(getContractAddress("safeStakeTokenRPL")), _amount);
    ...
    function slashRPL(address _nodeAddress, uint256 _ethSlashAmount) override external onlyLatestContract("safeStakeNodeStaking", address(this)) onlyRegisteredMinipool(msg.sender) {
        // Load contracts
        SafeStakeNetworkPricesInterface safeStakeNetworkPrices = SafeStakeNetworkPricesInterface(getContractAddress("safeStakeNetworkPrices"));
        SafeStakeVaultInterface safeStakeVault = SafeStakeVaultInterface(getContractAddress("safeStakeVault"));
        // Calculate RPL amount to slash
        uint256 rplSlashAmount = calcBase.mul(_ethSlashAmount).div(safeStakeNetworkPrices.getRPLPrice());
        ...
        if(rplSlashAmount > 0) safeStakeVault.transferToken("safeStakeAuctionManager", IERC20(getContractAddress("safeStakeTokenRPL")), rplSlashAmount);
        // Update RPL stake amounts
