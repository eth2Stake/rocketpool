...
import "./SafeStakeDAONodeTrustedSettings.sol";
...
contract SafeStakeDAONodeTrustedSettingsMinipool is SafeStakeDAONodeTrustedSettings, SafeStakeDAONodeTrustedSettingsMinipoolInterface {

    using SafeMath for uint;

    // Construct
    constructor(SafeStakeStorageInterface _safeStakeStorageAddress) SafeStakeDAONodeTrustedSettings(_safeStakeStorageAddress, "minipool") {
        // Set version
        version = 1;

        // If deployed during initial deployment, initialise now (otherwise must be called after upgrade)
        if (!_safeStakeStorageAddress.getDeployedStatus()){
            initialise();
        }
    }
    ...
    function setSettingUint(string memory _settingPath, uint256 _value) override public onlyDAONodeTrustedProposal {
        ...
        if(getBool(keccak256(abi.encodePacked(settingNameSpace, "deployed")))) {
            if(keccak256(abi.encodePacked(_settingPath)) == keccak256(abi.encodePacked("minipool.scrub.period"))) {
                ...
                require(_value <= (safeStakeDAOProtocolSettingsMinipool.getLaunchTimeout().sub(1 hours)), "Scrub period must be less than launch timeout");
            }
        }
        // Update setting now
