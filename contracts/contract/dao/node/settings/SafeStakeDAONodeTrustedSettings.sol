...
abstract contract SafeStakeDAONodeTrustedSettings is SafeStakeBase, SafeStakeDAONodeTrustedSettingsInterface {
    ...
    modifier onlyDAONodeTrustedProposal() {
        ...
        if(getBool(keccak256(abi.encodePacked(settingNameSpace, "deployed")))) require(getContractAddress("safeStakeDAONodeTrustedProposals") == msg.sender, "Only DAO Node Trusted Proposals contract can update a setting");
        _;
    }


    // Construct
    constructor(SafeStakeStorageInterface _safeStakeStorageAddress, string memory _settingNameSpace) SafeStakeBase(_safeStakeStorageAddress) {
