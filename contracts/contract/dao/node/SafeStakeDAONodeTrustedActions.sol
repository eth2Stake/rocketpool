...
import "../../../interface/dao/node/settings/SafeStakeDAONodeTrustedSettingsProposalsInterface.sol";
import "../../../interface/rewards/claims/SafeStakeClaimTrustedNodeInterface.sol";
...
contract SafeStakeDAONodeTrustedActions is SafeStakeBase, SafeStakeDAONodeTrustedActionsInterface {
    ...
    function _memberAdd(address _nodeAddress, uint256 _rplBondAmountPaid) private onlyRegisteredNode(_nodeAddress) {
        // Load contracts
        SafeStakeDAONodeTrustedInterface safeStakeDAONode = SafeStakeDAONodeTrustedInterface(getContractAddress("safeStakeDAONodeTrusted"));
        AddressSetStorageInterface addressSetStorage = AddressSetStorageInterface(getContractAddress("addressSetStorage"));
        // Check current node status
        require(safeStakeDAONode.getMemberIsValid(_nodeAddress) != true, "This node is already part of the trusted node DAO");
    ...
    function _memberJoin(address _nodeAddress) private {
        ...
        address safeStakeVaultAddress = getContractAddress("safeStakeVault");
        address safeStakeTokenRPLAddress = getContractAddress("safeStakeTokenRPL");
        // Load contracts
        IERC20 rplInflationContract = IERC20(safeStakeTokenRPLAddress);
        SafeStakeVaultInterface safeStakeVault = SafeStakeVaultInterface(safeStakeVaultAddress);
        SafeStakeDAONodeTrustedInterface safeStakeDAONode = SafeStakeDAONodeTrustedInterface(getContractAddress("safeStakeDAONodeTrusted"));
        SafeStakeDAONodeTrustedSettingsMembersInterface safeStakeDAONodeTrustedSettingsMembers = SafeStakeDAONodeTrustedSettingsMembersInterface(getContractAddress("safeStakeDAONodeTrustedSettingsMembers"));
        SafeStakeDAONodeTrustedSettingsProposalsInterface safeStakeDAONodeTrustedSettingsProposals = SafeStakeDAONodeTrustedSettingsProposalsInterface(getContractAddress("safeStakeDAONodeTrustedSettingsProposals"));
        // The time that the member was successfully invited to join the DAO
        uint256 memberInvitedTime = safeStakeDAONode.getMemberProposalExecutedTime("invited", _nodeAddress);
        ...
        uint256 rplBondAmount = safeStakeDAONodeTrustedSettingsMembers.getRPLBond();
        // Has their invite expired?
        require(memberInvitedTime.add(safeStakeDAONodeTrustedSettingsProposals.getActionTime()) > block.timestamp, "This node's invitation to join has expired, please apply again");
        // Verify they have allowed this contract to spend their RPL for the bond
        require(rplInflationContract.allowance(_nodeAddress, address(this)) >= rplBondAmount, "Not enough allowance given to SafeStakeDAONodeTrusted contract for transfer of RPL bond tokens");
        // Transfer the tokens to this contract now
        require(rplInflationContract.transferFrom(_nodeAddress, address(this), rplBondAmount), "Token transfer to SafeStakeDAONodeTrusted contract was not successful");
        // Allow SafeStakeVault to transfer these tokens to itself now
        require(rplInflationContract.approve(safeStakeVaultAddress, rplBondAmount), "Approval for SafeStakeVault to spend SafeStakeDAONodeTrusted RPL bond tokens was not successful");
        // Let vault know it can move these tokens to itself now and credit the balance to this contract
        safeStakeVault.depositToken(getContractName(address(this)), IERC20(safeStakeTokenRPLAddress), rplBondAmount);
    ...
    function actionJoin() override external onlyRegisteredNode(msg.sender) onlyLatestContract("safeStakeDAONodeTrustedActions", address(this)) {
    ...
    // They will be required to have the RPL bond amount in their account. This is called directly from SafeStakeDAONodeTrusted.
    function actionJoinRequired(address _nodeAddress) override external onlyRegisteredNode(_nodeAddress) onlyLatestContract("safeStakeDAONodeTrusted", msg.sender) {
    ...
    function actionLeave(address _rplBondRefundAddress) override external onlyTrustedNode(msg.sender) onlyLatestContract("safeStakeDAONodeTrustedActions", address(this)) {
        // Load contracts
        ...
        require(safeStakeDAONode.getMemberCount() > safeStakeDAONode.getMemberMinRequired(), "Member count will fall below min required");
        // Get the time that they were approved to leave at
        uint256 leaveAcceptedTime = safeStakeDAONode.getMemberProposalExecutedTime("leave", msg.sender);
        // Has their leave request expired?
        require(leaveAcceptedTime.add(safeStakeDAONodeTrustedSettingsProposals.getActionTime()) > block.timestamp, "This member has not been approved to leave or request has expired, please apply to leave again");
        // They were successful, lets refund their RPL Bond
        uint256 rplBondRefundAmount = safeStakeDAONode.getMemberRPLBondAmount(msg.sender);
        // Refund
        if(rplBondRefundAmount > 0) {
            ...
            // Send tokens now
            safeStakeVault.withdrawToken(_rplBondRefundAddress, IERC20(getContractAddress("safeStakeTokenRPL")), rplBondRefundAmount);
        }
        // Remove them now
    ...
    function actionKick(address _nodeAddress, uint256 _rplFine) override external onlyTrustedNode(_nodeAddress) onlyLatestContract("safeStakeDAONodeTrustedProposals", msg.sender) {
        // Load contracts
        SafeStakeVaultInterface safeStakeVault = SafeStakeVaultInterface(getContractAddress("safeStakeVault"));
        SafeStakeDAONodeTrustedInterface safeStakeDAONode = SafeStakeDAONodeTrustedInterface(getContractAddress("safeStakeDAONodeTrusted"));
        IERC20 rplToken = IERC20(getContractAddress("safeStakeTokenRPL"));
        // Get the
        uint256 rplBondRefundAmount = safeStakeDAONode.getMemberRPLBondAmount(_nodeAddress);
        // Refund
        if (rplBondRefundAmount > 0) {
            // Send tokens now if the vault can cover it
            if(rplToken.balanceOf(address(safeStakeVault)) >= rplBondRefundAmount) safeStakeVault.withdrawToken(_nodeAddress, IERC20(getContractAddress("safeStakeTokenRPL")), rplBondRefundAmount);
        }
        // Burn the fine
        if (_rplFine > 0) {
            safeStakeVault.burnToken(ERC20Burnable(getContractAddress("safeStakeTokenRPL")), _rplFine);
        }
    ...
    function actionChallengeMake(address _nodeAddress) override external onlyTrustedNode(_nodeAddress) onlyRegisteredNode(msg.sender) onlyLatestContract("safeStakeDAONodeTrustedActions", address(this)) payable {
        // Load contracts
        SafeStakeDAONodeTrustedInterface safeStakeDAONode = SafeStakeDAONodeTrustedInterface(getContractAddress("safeStakeDAONodeTrusted"));
        SafeStakeDAONodeTrustedSettingsMembersInterface safeStakeDAONodeTrustedSettingsMembers = SafeStakeDAONodeTrustedSettingsMembersInterface(getContractAddress("safeStakeDAONodeTrustedSettingsMembers"));
        ...
        if(safeStakeDAONode.getMemberIsValid(msg.sender) != true) require(msg.value == safeStakeDAONodeTrustedSettingsMembers.getChallengeCost(), "Non DAO members must pay ETH to challenge a members node");
        ...
        require(!safeStakeDAONode.getMemberIsChallenged(_nodeAddress), "Member is already being challenged");
        // Has this node recently made another challenge and not waited for the cooldown to pass?
        require(getUint(keccak256(abi.encodePacked(daoNameSpace, "node.challenge.created.time", msg.sender))).add(safeStakeDAONodeTrustedSettingsMembers.getChallengeCooldown()) < block.timestamp, "You must wait for the challenge cooldown to pass before issuing another challenge");
    ...
    function actionChallengeDecide(address _nodeAddress) override external onlyTrustedNode(_nodeAddress) onlyRegisteredNode(msg.sender) onlyLatestContract("safeStakeDAONodeTrustedActions", address(this)) {
        // Load contracts
        SafeStakeDAONodeTrustedSettingsMembersInterface safeStakeDAONodeTrustedSettingsMembers = SafeStakeDAONodeTrustedSettingsMembersInterface(getContractAddress("safeStakeDAONodeTrustedSettingsMembers"));
        ...
        if(_nodeAddress == msg.sender) {
            ...
            require(challengeTime.add(safeStakeDAONodeTrustedSettingsMembers.getChallengeWindow()) < block.timestamp, "Refute window has not yet passed");
