...
contract SafeStakeDAONodeTrusted is SafeStakeBase, SafeStakeDAONodeTrustedInterface {
    ...
    function getMemberQuorumVotesRequired() override external view returns (uint256) {
        ...
        return getMemberCount().mul(safeStakeDAONodeTrustedSettingsMembers.getQuorum());
    }
    ...
    // Only accepts calls from the SafeStakeMinipoolManager contract
    function incrementMemberUnbondedValidatorCount(address _nodeAddress) override external onlyLatestContract("safeStakeDAONodeTrusted", address(this)) onlyLatestContract("safeStakeMinipoolManager", msg.sender) {
        addUint(keccak256(abi.encodePacked(daoNameSpace, "member.validator.unbonded.count", _nodeAddress)), 1);
    }
    function decrementMemberUnbondedValidatorCount(address _nodeAddress) override external onlyLatestContract("safeStakeDAONodeTrusted", address(this)) onlyRegisteredMinipool(msg.sender) {
    ...
    function bootstrapSettingUint(string memory _settingContractName, string memory _settingPath, uint256 _value) override external onlyGuardian onlyBootstrapMode onlyLatestContract("safeStakeDAONodeTrusted", address(this)) {
        // Ok good to go, lets update the settings 
        SafeStakeDAONodeTrustedProposalsInterface(getContractAddress("safeStakeDAONodeTrustedProposals")).proposalSettingUint(_settingContractName, _settingPath, _value);
    }

    // Bootstrap mode - Bool Setting
    function bootstrapSettingBool(string memory _settingContractName, string memory _settingPath, bool _value) override external onlyGuardian onlyBootstrapMode onlyLatestContract("safeStakeDAONodeTrusted", address(this)) {
        // Ok good to go, lets update the settings 
        SafeStakeDAONodeTrustedProposalsInterface(getContractAddress("safeStakeDAONodeTrustedProposals")).proposalSettingBool(_settingContractName, _settingPath, _value);
    }


    // Bootstrap mode - Upgrade contracts or their ABI
    function bootstrapUpgrade(string memory _type, string memory _name, string memory _contractAbi, address _contractAddress) override external onlyGuardian onlyBootstrapMode onlyLatestContract("safeStakeDAONodeTrusted", address(this)) {
        // Ok good to go, lets update the settings 
        SafeStakeDAONodeTrustedProposalsInterface(getContractAddress("safeStakeDAONodeTrustedProposals")).proposalUpgrade(_type, _name, _contractAbi, _contractAddress);
    }

    // Bootstrap mode - Disable RP Access (only RP can call this to hand over full control to the DAO)
    function bootstrapDisable(bool _confirmDisableBootstrapMode) override external onlyGuardian onlyBootstrapMode onlyLatestContract("safeStakeDAONodeTrusted", address(this)) {
