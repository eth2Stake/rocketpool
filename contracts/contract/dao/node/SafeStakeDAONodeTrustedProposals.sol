...
import "../../../interface/dao/node/SafeStakeDAONodeTrustedUpgradeInterface.sol";
import "../../../interface/dao/node/settings/SafeStakeDAONodeTrustedSettingsInterface.sol";
...
contract SafeStakeDAONodeTrustedProposals is SafeStakeBase, SafeStakeDAONodeTrustedProposalsInterface {  
    ...
    modifier onlyExecutingContracts() {
        ...
        require(msg.sender == getContractAddress("safeStakeDAONodeTrusted") || msg.sender == getContractAddress("safeStakeDAOProposal"), "Sender is not permitted to access executing methods");
        _;
    }
    ...
    function propose(string memory _proposalMessage, bytes memory _payload) override external onlyTrustedNode(msg.sender) onlyLatestContract("safeStakeDAONodeTrustedProposals", address(this)) returns (uint256) {
        // Load contracts
        SafeStakeDAOProposalInterface daoProposal = SafeStakeDAOProposalInterface(getContractAddress("safeStakeDAOProposal"));
        SafeStakeDAONodeTrustedInterface daoNodeTrusted = SafeStakeDAONodeTrustedInterface(getContractAddress("safeStakeDAONodeTrusted"));
        ...
        require(daoNodeTrusted.getMemberLastProposalTime(msg.sender).add(safeStakeDAONodeTrustedSettingsProposals.getCooldownTime()) <= block.timestamp, "Member has not waited long enough to make another proposal");
        ...
        // Create the proposal
        return daoProposal.add(msg.sender, "safeStakeDAONodeTrustedProposals", _proposalMessage, block.timestamp.add(safeStakeDAONodeTrustedSettingsProposals.getVoteDelayTime()), safeStakeDAONodeTrustedSettingsProposals.getVoteTime(), safeStakeDAONodeTrustedSettingsProposals.getExecuteTime(), daoNodeTrusted.getMemberQuorumVotesRequired(), _payload);
    }

    // Vote on a proposal
    function vote(uint256 _proposalID, bool _support) override external onlyTrustedNode(msg.sender) onlyLatestContract("safeStakeDAONodeTrustedProposals", address(this)) {
        // Load contracts
        SafeStakeDAOProposalInterface daoProposal = SafeStakeDAOProposalInterface(getContractAddress("safeStakeDAOProposal"));
        SafeStakeDAONodeTrustedInterface daoNodeTrusted = SafeStakeDAONodeTrustedInterface(getContractAddress("safeStakeDAONodeTrusted"));
    ...
    // Cancel a proposal 
    function cancel(uint256 _proposalID) override external onlyTrustedNode(msg.sender) onlyLatestContract("safeStakeDAONodeTrustedProposals", address(this)) {
        // Load contracts
        SafeStakeDAOProposalInterface daoProposal = SafeStakeDAOProposalInterface(getContractAddress("safeStakeDAOProposal"));
    ...
    // Execute a proposal 
    function execute(uint256 _proposalID) override external onlyLatestContract("safeStakeDAONodeTrustedProposals", address(this)) {
        // Load contracts
        SafeStakeDAOProposalInterface daoProposal = SafeStakeDAOProposalInterface(getContractAddress("safeStakeDAOProposal"));
        // Execute now
    ...
    //     // They are initially only invited to join, so their membership isn't set as true until they accept it in SafeStakeDAONodeTrustedActions
    ...
    //     // Load contracts
    //     SafeStakeDAONodeTrustedInterface daoNodeTrusted = SafeStakeDAONodeTrustedInterface(getContractAddress("safeStakeDAONodeTrusted"));
    //     SafeStakeDAONodeTrustedActionsInterface daoActionsContract = SafeStakeDAONodeTrustedActionsInterface(getContractAddress("safeStakeDAONodeTrustedActions"));
    ...
    function proposalSettingUint(string memory _settingContractName, string memory _settingPath, uint256 _value) override external onlyExecutingContracts() {
        // Load contracts
        SafeStakeDAONodeTrustedSettingsInterface safeStakeDAONodeTrustedSettings = SafeStakeDAONodeTrustedSettingsInterface(getContractAddress(_settingContractName));
        // Lets update
        safeStakeDAONodeTrustedSettings.setSettingUint(_settingPath, _value);
    }
    ...
    function proposalSettingBool(string memory _settingContractName, string memory _settingPath, bool _value) override external onlyExecutingContracts() {
        ...
        SafeStakeDAONodeTrustedSettingsInterface safeStakeDAONodeTrustedSettings = SafeStakeDAONodeTrustedSettingsInterface(getContractAddress(_settingContractName));
        // Lets update
        safeStakeDAONodeTrustedSettings.setSettingBool(_settingPath, _value);
    }
    ...
    function proposalUpgrade(string memory _type, string memory _name, string memory _contractAbi, address _contractAddress) override external onlyExecutingContracts() {
        // Load contracts
        SafeStakeDAONodeTrustedUpgradeInterface safeStakeDAONodeTrustedUpgradeInterface = SafeStakeDAONodeTrustedUpgradeInterface(getContractAddress("safeStakeDAONodeTrustedUpgrade"));
        // Lets update
        safeStakeDAONodeTrustedUpgradeInterface.upgrade(_type, _name, _contractAbi, _contractAddress);
    }
