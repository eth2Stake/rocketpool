...
// The SafeStake Pool Network DAO - This is a placeholder for the network DAO to come
contract SafeStakeDAOProtocol is SafeStakeBase, SafeStakeDAOProtocolInterface {
    ...
    function bootstrapSettingMulti(string[] memory _settingContractNames, string[] memory _settingPaths, SettingType[] memory _types, bytes[] memory _values) override external onlyGuardian onlyBootstrapMode onlyLatestContract("safeStakeDAOProtocol", address(this)) {
      // Ok good to go, lets update the settings
      SafeStakeDAOProtocolProposalsInterface(getContractAddress("safeStakeDAOProtocolProposals")).proposalSettingMulti(_settingContractNames, _settingPaths, _types, _values);
    }

    // Bootstrap mode - Uint Setting
    function bootstrapSettingUint(string memory _settingContractName, string memory _settingPath, uint256 _value) override external onlyGuardian onlyBootstrapMode onlyLatestContract("safeStakeDAOProtocol", address(this)) {
        // Ok good to go, lets update the settings
        SafeStakeDAOProtocolProposalsInterface(getContractAddress("safeStakeDAOProtocolProposals")).proposalSettingUint(_settingContractName, _settingPath, _value);
    }

    // Bootstrap mode - Bool Setting
    function bootstrapSettingBool(string memory _settingContractName, string memory _settingPath, bool _value) override external onlyGuardian onlyBootstrapMode onlyLatestContract("safeStakeDAOProtocol", address(this)) {
        // Ok good to go, lets update the settings 
        SafeStakeDAOProtocolProposalsInterface(getContractAddress("safeStakeDAOProtocolProposals")).proposalSettingBool(_settingContractName, _settingPath, _value);
    }

    // Bootstrap mode - Address Setting
    function bootstrapSettingAddress(string memory _settingContractName, string memory _settingPath, address _value) override external onlyGuardian onlyBootstrapMode onlyLatestContract("safeStakeDAOProtocol", address(this)) {
        // Ok good to go, lets update the settings 
