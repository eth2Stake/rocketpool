...
contract SafeStakeDAOProtocolSettingsRewards is SafeStakeDAOProtocolSettings, SafeStakeDAOProtocolSettingsRewardsInterface {

    using SafeMath for uint;

    // Construct
    constructor(SafeStakeStorageInterface _safeStakeStorageAddress) SafeStakeDAOProtocolSettings(_safeStakeStorageAddress, "rewards") {
        // Set version 
        version = 1;
         // Set some initial settings on first deployment
        if(!getBool(keccak256(abi.encodePacked(settingNameSpace, "deployed")))) {
            // Each of the initial RPL reward claiming contracts
            setSettingRewardsClaimer('safeStakeClaimDAO', 0.1 ether);                                              // DAO Rewards claim % amount - Percentage given of 1 ether
            setSettingRewardsClaimer('safeStakeClaimNode', 0.70 ether);                                            // Bonded Node Rewards claim % amount - Percentage given of 1 ether
            setSettingRewardsClaimer('safeStakeClaimTrustedNode', 0.2 ether);                                      // Trusted Node Rewards claim % amount - Percentage given of 1 ether
            // RPL Claims settings
