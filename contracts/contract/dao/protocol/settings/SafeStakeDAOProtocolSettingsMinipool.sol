...
contract SafeStakeDAOProtocolSettingsMinipool is SafeStakeDAOProtocolSettings, SafeStakeDAOProtocolSettingsMinipoolInterface {

    // Libs
    using SafeMath for uint;

    // Construct
    constructor(SafeStakeStorageInterface _safeStakeStorageAddress) SafeStakeDAOProtocolSettings(_safeStakeStorageAddress, "minipool") {
        // Set version
        version = 1;
    ...
    function setSettingUint(string memory _settingPath, uint256 _value) override public onlyDAOProtocolProposal {
        ...
        if(getBool(keccak256(abi.encodePacked(settingNameSpace, "deployed")))) {
            if(keccak256(abi.encodePacked(_settingPath)) == keccak256(abi.encodePacked("minipool.launch.timeout"))) {
                ...
                require(_value >= (safeStakeDAONodeTrustedSettingsMinipool.getScrubPeriod().add(1 hours)), "Launch timeout must be greater than scrub period");
            }
        }
        // Update setting now
