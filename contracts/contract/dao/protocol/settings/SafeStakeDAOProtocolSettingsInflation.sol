...
contract SafeStakeDAOProtocolSettingsInflation is SafeStakeDAOProtocolSettings, SafeStakeDAOProtocolSettingsInflationInterface {

    // Construct
    constructor(SafeStakeStorageInterface _safeStakeStorageAddress) SafeStakeDAOProtocolSettings(_safeStakeStorageAddress, "inflation") {
        // Set version 
        version = 1;
    ...
    function setSettingUint(string memory _settingPath, uint256 _value) override public onlyDAOProtocolProposal {
        ...
        if(settingKey == keccak256(bytes("rpl.inflation.interval.start"))) {
            ...
            address rplContractAddress = getContractAddressUnsafe("safeStakeTokenRPL");
            if(rplContractAddress != address(0x0)) {
                // Force inflation at old rate before updating inflation rate
                SafeStakeTokenRPLInterface rplContract = SafeStakeTokenRPLInterface(rplContractAddress);
