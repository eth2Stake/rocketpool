...
contract SafeStakeNetworkPrices is SafeStakeBase, SafeStakeNetworkPricesInterface {
    ...
    function submitPrices(uint256 _block, uint256 _rplPrice, uint256 _effectiveRplStake) override external onlyLatestContract("safeStakeNetworkPrices", address(this)) onlyTrustedNode(msg.sender) {
        // Check settings
        ...
        require(safeStakeDAOProtocolSettingsNetwork.getSubmitPricesEnabled(), "Submitting prices is currently disabled");
        // Check block
    ...
    function executeUpdatePrices(uint256 _block, uint256 _rplPrice, uint256 _effectiveRplStake) override external onlyLatestContract("safeStakeNetworkPrices", address(this)) {
        // Check settings
        SafeStakeDAOProtocolSettingsNetworkInterface safeStakeDAOProtocolSettingsNetwork = SafeStakeDAOProtocolSettingsNetworkInterface(getContractAddress("safeStakeDAOProtocolSettingsNetwork"));
        require(safeStakeDAOProtocolSettingsNetwork.getSubmitPricesEnabled(), "Submitting prices is currently disabled");
        // Check block
    ...
    function inConsensus() override public view returns (bool) {
        ...
        uint256 updateFrequency = safeStakeDAOProtocolSettingsNetwork.getSubmitPricesFrequency();
