...
#. Ensure your Eth 1.0 and/or Eth 2.0 clients are listening on the address ``0.0.0.0``:
    ...
    * Geth: ``--http --http.addr 0.0.0.0 --http.port 8585 --http.vhosts *``
...
#. Open ``~/.rocketpool/config.yml``, and make the following changes:
    ...
    * To use your own Geth instance, update ``chains.eth1.provider`` to ``http://XXX.XXX.XXX.XXX:8585``, where ``XXX.XXX.XXX.XXX`` is your machine's local IP address
