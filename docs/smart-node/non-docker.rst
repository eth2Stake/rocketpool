...
#. Make the following modifications to your Rocket Pool config file (``~/.rocketpool/config.yml``):
    ...
    * Update ``chains.eth1.provider`` to e.g. ``http://127.0.0.1:8585``
...
Geth::
    ...
    Restart=always
    RestartSec=5
    ExecStart=/path/to/geth --goerli --http --http.addr 127.0.0.1 --http.port 8585 --http.api eth,net,personal,web3 --http.vhosts *
...
Lighthouse - Beacon Chain::
    ...
    Restart=always
    RestartSec=5
    ExecStart=/path/to/lighthouse beacon --testnet medalla --eth1 --eth1-endpoint http://127.0.0.1:8585 --http --http-address 127.0.0.1 --http-port 5052
...
Prysm - Beacon Chain::
    ...
    Restart=always
    RestartSec=5
    ExecStart=/path/to/prysm/beacon-chain --accept-terms-of-use --medalla --http-web3provider http://127.0.0.1:8585 --rpc-host 127.0.0.1 --rpc-port 5052
